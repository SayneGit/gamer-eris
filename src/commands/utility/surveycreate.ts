...
import { deleteMessage, needMessage, sendMessage } from '../../lib/utils/eris'
...
export default new Command(['surveycreate'], async (message, args, context) => {
  ...
  const questionMessage = await sendMessage(
    message.channel.id,
    language('utility/surveycreate:NEED_NAME', { mention: message.author.mention })
  )
  ...
  const name = await needMessage(message)
  if (CANCEL_OPTIONS.includes(name.content.toLowerCase()))
    return sendMessage(message.channel.id, language('utility/surveycreate:CANCELLED'))

  const exists = await Gamer.database.models.survey.findOne({
    name: name.content.toLowerCase(),
    guildID: message.member.guild.id
  })
  if (exists) return sendMessage(message.channel.id, language('utility/surveycreate:NAME_TAKEN', { name: name.content }))

	payload.name = name.content.toLowerCase()
	
  await questionMessage.edit(language('utility/surveycreate:NEED_RESULTS_CHANNEL'))

  const resultsChannelResponse = await needMessage(message)
  if (CANCEL_OPTIONS.includes(resultsChannelResponse.content.toLowerCase()))
    return sendMessage(message.channel.id, language('utility/surveycreate:CANCELLED'))

  await questionMessage.edit(language('utility/surveycreate:'))
    ...
    callback: async (msg, collector) => {
      ...
      switch (data.step) {
        ...
        case 2:
          ...
          data.options.push(msg.scontent)
          questionMessage.edit(
